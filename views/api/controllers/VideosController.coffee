...
module.exports =
  ...
  list: (req, res) ->
    ...
    Video.find().sort('title ASC').exec (err, videos) ->
  ...
  getMetadata: (req, res) ->
    ...
    Video.findOne({id: req.params.id}).exec (err, video) ->
      return res.send(err, 500) if (err)
  ...
  stream: (req, res) ->
    ...
    Video.findOne(req.params.id).exec (err, video) ->
      return res.send(err, 500) if err
  ...
  watch: (req, res) ->
    Video.findOne(req.param('id')).exec (err, video) ->
      return res.send(err, 500) if err
